...
const jsonDbPath = __dirname + '/../data/film.json';
...
router.get('/', (req, res) => {
    ...
    let filterdefaultFilms = [...films].filter((film) =>
        film.duration >= minDurationValue
    );
    res.json(filterdefaultFilms);
...
router.delete('/:id', (req, res) => {
    const films = parse(jsonDbPath,defaultFilms);
    const indexdefaultFilms = films.findIndex((film) =>
        film.id === Number(req.params.id));
